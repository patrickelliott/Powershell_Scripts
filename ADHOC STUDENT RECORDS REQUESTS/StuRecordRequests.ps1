# 
# Populate UD Table with Student Record Requests
#

function readSourceAndUpdateDatabase{
$counter = 1
$fileA = "C:\Temp\StuRecordRequests.txt"
...
if ($counter -gt 1) {
	...
	#display on screen...
	 ...
	 if ($spldate[0].length -eq 1){$mo = "0"+$spldate[0]}
	 if ($spldate[1].length -eq 1){$dy = "0"+$spldate[1]}
	 $frmtdate = $spldate[2] + $mo + $dy 
	 
	 #$date = 7/4/2004
	 $rqdate = [datetime]::ParseExact($frmtdate, "yyyymmdd", $null)
	 
		
#Update
#PROD
$Cmd.CommandText = "INSERT INTO rev.UD_RECORDREQUEST(UDRECORDREQUEST_GU,REQUEST_DATE,REQUEST_TYPE,REQUESTING_AGENCY,STUDENT_GU) VALUES (`'$guid`',`'$rqdate`',`'$rqtype`',`'$rqagency`',(select student_gu from rev.epc_stu where sis_number = `'$sisnum`'))"
$Cmd.ExecuteNonQuery()
