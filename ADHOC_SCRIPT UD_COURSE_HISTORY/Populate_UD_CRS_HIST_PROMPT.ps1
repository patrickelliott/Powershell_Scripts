# 
# Populate UD Table with Student Course History
#

function readSourceAndUpdateDatabase{
$counter = 1
$fileA = "C:\Temp\STUDENTCOURSEHISTORY.txt"
...
if ($counter -gt 1) {
	...
	#display on screen...
	 ...
	 $enddate = [datetime]::ParseExact($clsenddate, "yyyymmdd", $null)

	 if ($atttot -eq ""){$atttot = 0} 
	 if ($crdatt -eq ""){$crdatt = 0} 
	 if ($crdcomp -eq ""){$crdcomp = 0} 
	 
	 if ($histype -eq "1"){$histype = "Middle School"} 
	 else {$histype = "High School"} 
	 
	 if ($indistsch -eq "10153"){$indistsch = "Resource Link"}
		elseif($indistsch -eq "10155"){$indistsch = "Coos Bay Alternative Programs"}
		elseif($indistsch -eq "10156"){$indistsch = "CCYOP"}
		elseif($indistsch -eq "197"){$indistsch = "Millicoma"}
		elseif($indistsch -eq "199"){$indistsch = "Sunset"}
		elseif($indistsch -eq "290"){$indistsch = "Marshfield"}
		elseif($indistsch -eq "3227"){$indistsch = "Destinations"}
		else{$indistsch = ""}
		
#Update
#PROD
$Cmd.CommandText = "INSERT INTO rev.UD_STU_CRS_HISTORY (UDSTU_CRS_HISTORY_GU,ATTENDANCE_TOTAL,CALENDAR_MONTH,CALENDAR_YEAR,CLASS_END_DATE,COURSE_ID,COURSE_TITLE,CREDIT_ATTEMPTED,CREDIT_COMPLETED,CRS_HISTORY_TYPE,DISTRICT_COURSE_ID,GRADE,IN_DISTRICT_SCHOOL,MARK,SCHOOL_YEAR,STUDENT_GU,TEACHER_NAME) VALUES (`'$guid`',`'$atttot`',`'$calmon`',`'$calyr`',`'$enddate`',`'$crsid`',`'$crstitle`',`'$crdatt`',`'$crdcomp`',`'$histype`',`'$distcrsid`',`'$grade`',`'$indistsch`',`'$mark`',`'$schyr`',(select student_gu from rev.epc_stu where sis_number = `'$sisnum`'),`'$tchname`')"
