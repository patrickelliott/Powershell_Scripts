# 
# Populate Student LEP
#

function readSourceAndUpdateDatabase{
$counter = 1
$fileA = "C:\Temp\STUDENTLEP.txt"
...
#Update
$Cmd.CommandText = "UPDATE rev.EPC_STU_PGM_ELL set LEP_US_START_DATE = `'$enterdate`', RECLASSIFICATION_DATE = `'$exitdate`' where student_gu = (select distinct student_gu from rev.EPC_STU where SIS_NUMBER = `'$sisnum`')"
$Cmd.ExecuteNonQuery()

 }
$counter++ 
}
#when I directly inject records, if the date is null, it'll put in a bogus date. This will remove them...
$Cmd.CommandText = "UPDATE rev.EPC_STU_PGM_ELL set RECLASSIFICATION_DATE = NULL where RECLASSIFICATION_DATE = '1900-01-01 00:00:00'"
$Cmd.ExecuteNonQuery()
