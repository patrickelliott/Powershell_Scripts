# 
# Populate Student Dentist information
#

function readSourceAndUpdateDatabase{
$counter = 1
$fileA = "C:\Temp\Dentist_List.txt"
...
#Update
$Cmd.CommandText = "UPDATE rev.EPC_STU_PHYSICIAN set DENTAL_NAME = `'$name`', DENTAL_PHONE = `'$phone`', HAS_INSURANCE = `'$hasins`', INSURANCE_COMPANY =  `'$insco`' where student_gu = (select distinct student_gu from rev.EPC_STU where SIS_NUMBER = `'$sisnum`')"
$Cmd.ExecuteNonQuery()
